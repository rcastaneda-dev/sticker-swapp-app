...
│   ├── migrations/
│   └── functions/
│       ├── ably-webhook/        # Ably Reactor webhook → messages table
│       └── ...
...
make test-integration # Run trade integration tests (requires local Supabase: supabase start)
...
| Channel Pattern                     | Permissions              | Purpose            | Events |
|-------------------------------------|--------------------------|---------------------|--------|
| `match:{matchId}`                   | publish, subscribe, presence, history | Trade chat + confirmation | `chat.message`, `match.created`, `trade.confirmed` |
...
| POST   | `/api/v1/matches/{matchId}/confirm` | JWT | Required | Required | 120/min | Record caller's trade confirmation |
| GET    | `/api/v1/matches/{matchId}/confirm` | JWT | Required | Required | 120/min | Get trade confirmation state |
...
### Proximity Query Load Test Results

**Status: GO** — server-side p95 = **0.9 ms** (target: <100 ms). Optimized via denormalized inventory counts on `user_profiles`.

**Test parameters:** 1000 users seeded around San Salvador (13.6929, -89.2182) with 25 km spread, 50 stickers/user (40% OWNED, 30% DUPLICATE, 30% NEEDED), 5 km query radius.

**Optimization benchmark (EXPLAIN ANALYZE, 200 samples each):**

| Variant | p50 | p95 | Improvement | Result |
|---------|-----|-----|-------------|--------|
| Baseline (inline GROUP BY) | 151.5 ms | 188.2 ms | — | FAIL |
| A: LATERAL subquery | 1.2 ms | 1.4 ms | 134x | PASS |
| **B: Denormalized columns** | **0.8 ms** | **0.9 ms** | **209x** | **PASS (winner)** |
| C: Materialized view | 0.9 ms | 1.0 ms | 188x | PASS |

**Applied optimization (migration 0029):** `inventory_duplicate_count` and `inventory_needed_count` columns on `user_profiles`, maintained by `trg_maintain_inventory_counts` row-level trigger on `user_inventory` (fires on INSERT, UPDATE of status, DELETE). The `find_nearby_traders()` RPC now joins directly to `user_profiles` columns with zero aggregation at query time.

**Run command:** `make test-loadtest`

**Key files:**
- `go_service/internal/matchmaking/loadtest_test.go` — Load test (integration tag, EXPLAIN ANALYZE benchmarking)
...
1. Idempotency check — if `idempotency_key` already exists in `trade_audit_log`, return full trade response (trade_id, match_id, participant IDs, sticker arrays, status) with `already_completed: true`
...
8. Audit — calls `record_trade()` with COMPLETED status and match_id
...
- `supabase/migrations/0022_enrich_trade_idempotency.sql` — Adds `match_id` to `trade_audit_log`, enriches idempotent replay to return full response
...
**Route:** `/matches/:matchId` — Trade confirmation UI with "Mark as Done" flow. Both matched users must confirm to complete the trade. Under-13 users see `SwappRestrictedEmptyState`.

**Flow:**
1. Users match via swiping and chat to coordinate a physical sticker trade
2. Both users meet in person to swap physical stickers
3. User A taps "Mark as Done" → `POST /api/v1/matches/{matchId}/confirm` → Go backend stamps `user1_confirmed_at` or `user2_confirmed_at`
4. Go backend publishes `trade.confirmed` event to `match:{matchId}` Ably channel
5. User B sees real-time update (partner confirmed), taps "Mark as Done"
6. When both confirmed → match status auto-transitions to COMPLETED → success animation

**State machine:** `loading → idle → confirming → waitingForOther → completed` (also `error` with retry)

**Confirmation chips:** Two visual chips ("You" / "Partner") show confirmed/unconfirmed state with check/circle icons.

**Real-time sync:** `trade.confirmed` Ably event on `match:{matchId}` channel updates the other user's UI in real-time. Initial state fetched from `GET /api/v1/matches/{matchId}/confirm` on screen load.

**Concurrency safety:** `FOR UPDATE` row lock in `confirm_trade_completion()` RPC prevents race conditions when both users confirm simultaneously. Confirmation is idempotent (`COALESCE` preserves first timestamp).

**Providers:**
- `tradeConfirmationServiceProvider` — `Provider<TradeConfirmationService>` singleton
- `tradeConfirmationProvider` — `NotifierProvider.family<TradeConfirmationNotifier, TradeConfirmationState, String>` (matchId). State machine: loading → idle/waitingForOther/completed/error. Methods: `initialize()`, `confirm()`.

**Database (migration `0024`):**
- Added `user1_confirmed_at timestamptz` and `user2_confirmed_at timestamptz` to `matches` table
- `confirm_trade_completion(p_match_id)` SECURITY DEFINER RPC: validates participant + PENDING status, stamps caller's confirmation, auto-transitions to COMPLETED when both confirmed. Returns `{success, caller_confirmed, other_confirmed, both_confirmed, match_status}`.
- `get_trade_confirmation_state(p_match_id)` SECURITY DEFINER RPC: read-only version returning same shape.

**Key files:**
- `supabase/migrations/0024_add_trade_confirmation.sql` — Columns + RPCs
- `go_service/internal/trades/confirmer.go` — `TradeConfirmer` interface, `ConfirmResult`
- `go_service/internal/trades/db_confirmer.go` — `DBTradeConfirmer` pgx implementation
- `go_service/internal/api/confirm.go` — `ConfirmHandler` with `ConfirmTrade` and `GetConfirmationState` HTTP handlers
- `flutter_app/lib/features/matching/data/services/trade_confirmation_service.dart` — HTTP service
- `flutter_app/lib/features/matching/data/providers/trade_confirmation_providers.dart` — Riverpod state management
- `flutter_app/lib/features/matching/presentation/screens/match_screen.dart` — Confirmation UI
...
## Integration Tests (Trade Execution)

**Build tag:** `//go:build integration` — separated from unit tests, only runs with `-tags=integration`.

**Prerequisites:** Supabase database accessible via **direct connection** (not pooler — advisory locks require session mode). Set `TEST_DATABASE_URL` in `.env` or export it. Format: `postgresql://postgres.[ref]:[password]@aws-0-[region].pooler.supabase.com:5432/postgres` (port **5432** = direct, not 6543 = pooler). Falls back to `SUPABASE_DB_URL` if `TEST_DATABASE_URL` is unset.

**Command:** `make test-integration` (runs with `-race -count=1 -timeout=120s`)

**Test cases:**
1. **HappyPath** — Single trade executes correctly: stickers transfer, match→COMPLETED, 1 audit record, locks released with TRADE_COMPLETED
2. **100Concurrent_ZeroDoubleTrades** — 100 goroutines fire `ExecuteTrade` simultaneously on same match (two-phase `sync.WaitGroup` barrier). Asserts: exactly 1 success, 99 get MATCH_NOT_PENDING, exactly 1 audit record, sticker quantities changed once
3. **Idempotency** — Same idempotency key returns `AlreadyCompleted=true` with identical `TradeID`, enriched response fields (migration 0022), exactly 1 audit record
4. **SharedSticker_TwoMatches** — Alice's sticker locked for Match A; lock attempt for Match B returns `STICKERS_NOT_AVAILABLE`. Proves locking layer prevents double-spending across matches

**Test utilities** (`go_service/internal/testutil/testutil.go`):
- `MustPool(t, maxConns)` — pool with configurable max connections, `t.Skip` if no DB URL
- `CreateTestUser(ctx, pool, email)` — direct `auth.users` INSERT (triggers `user_profiles` auto-creation)
- `SeedInventory(ctx, pool, userID, items)` — batch `user_inventory` INSERT with ON CONFLICT
- `CreateMatch(ctx, pool, user1ID, user2ID)` — PENDING match with canonical ordering
- `LockInventory(ctx, pool, userID, matchID)` — calls `lock_inventory_for_trade()` RPC via transaction with `set_config`
- `LockInventoryRaw(ctx, pool, userID, matchID)` — returns success/error code without failing (for testing expected lock failures)
- `Cleanup(ctx, pool, userIDs)` — DELETE in reverse FK order
- `NewUUID()` — inline UUID v4 generator (no external dependency)

**Key files:**
- `go_service/internal/testutil/testutil.go` — Shared DB helpers
- `go_service/internal/trades/integration_test.go` — 4 integration test cases
- `go_service/internal/matchmaking/loadtest_test.go` — PostGIS proximity query load test (see Proximity Query Load Test Results)
...
- `user_profiles` — Trust Score, age verification (`date_of_birth`, `is_under_13`, `age_verified_at` — immutable after verification), preferences. `inventory_duplicate_count`/`inventory_needed_count` (migration `0028`) denormalized from `user_inventory`, maintained by `trg_maintain_inventory_counts` trigger.
...
- `trade_audit_log` — Immutable trade history (append-only via `record_trade()` SECURITY DEFINER function, RLS read-only for participants). Includes `match_id` (migration `0022`) for full idempotent replay responses.
...
- `matches` — Created when both users swipe right on each other. UUID primary key (used in Ably channel names). Canonical ordering (`CHECK user1_id < user2_id`) with UNIQUE constraint prevents duplicate A↔B matches. Status: `match_status` enum (PENDING/ACCEPTED/COMPLETED/CANCELLED/EXPIRED). `user1_confirmed_at` and `user2_confirmed_at` (migration `0024`) track trade confirmation timestamps — when both are set, match auto-transitions to COMPLETED. RLS read-only for participants; writes via `create_match_if_mutual()` and `confirm_trade_completion()` SECURITY DEFINER RPCs.
...
- `ably-webhook` — Ably Reactor webhook endpoint (POST). Receives message envelopes from Ably when messages are published on `match:*` channels. Verifies HMAC-SHA256 signature (`X-Ably-Signature` header) using `ABLY_API_KEY` secret. Filters to `chat.message` events only (skips system events like `match.created`). Extracts `match_id` from channel name and `sender_id` from `clientId`. Inserts via `insert_message()` RPC (service_role). Idempotent: duplicate Ably message IDs are silently accepted. Always returns 200 to prevent Ably retries on partial failures.
...
ABLY_API_KEY              # Ably API key (server-only; also set as Edge Function secret for webhook signature verification)
