-- Find nearby users who have duplicate stickers available for trade.
-- Extends find_nearby_users() by joining user_inventory to filter for active
-- traders (at least one DUPLICATE sticker).  Uses the PostGIS <-> operator
-- for index-accelerated nearest-neighbour ordering.
